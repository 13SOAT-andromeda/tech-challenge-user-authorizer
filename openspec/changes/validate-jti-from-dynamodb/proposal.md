...
- **Update config/env**: `SESSION_TABLE_NAME` default changes from `user-sessions` to `user-authentication-token`.
- **Update Makefile & deploy scripts**: align DynamoDB table name and seed commands with `user-authentication-token` schema.
...
- **`internal/config/config.go`** — update default table name from `user-sessions` to `user-authentication-token`
- **`Makefile`** — update DynamoDB bootstrap targets to use `user-authentication-token` schema
